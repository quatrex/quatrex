...
template <typename T>
__device__ inline T make_conj(T x) { return x; }

template <>
__device__ inline complex<double> make_conj(complex<double> x) {
    return conj(x);
}

template<typename T1, typename T2, typename T3> 
__global__ void _scatter_add_scaled(T1* M, const T2* U,
...
_scatter_add_scaled_obc = cp.RawKernel(
    r"""
#include <cupy/complex.cuh>
extern "C" __global__
void _scatter_add_scaled_obc(complex<double>* M, const complex<double>* S, const double K1, const double K2, const int N_S_big, const int N_S, const int n_rep_2,
            const long* ind, const int N, const double alpha) {
