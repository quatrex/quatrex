...
def _load_matrix(
    ...
    kpoint_grid = np.array(quatrex_config.device.kpoint_grid)
    kpoints = monkhorst_pack(kpoint_grid, quatrex_config.device.kpoint_shift).reshape(
        tuple(kpoint_grid) + (-1,)
    )
    if matrix_name == "coulomb_matrix":
        kpoints -= kpoints[0, 0, 0] # Shift the gamma point to zero for the Coulomb matrix.
    _assemble_kpoint(
        matrix,
        matrix_dict,
        kpoints,
    ...
    # num_kpoints = np.array(quatrex_config.device.kpoint_grid)
    # if matrix_name == "coulomb_matrix":
