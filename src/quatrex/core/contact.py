...
class Contact:
    ...
    def _get_atoms_inside_cell(self, nx:int ,ny: int, nz: int) -> NDArray:
        ...
        inside_mask = xp.nonzero(
            (coeffs[:, 0] >= nx) & (coeffs[:, 0] <= nx+1) &
            (coeffs[:, 1] >= ny) & (coeffs[:, 1] <= ny+1) &
            (coeffs[:, 2] >= nz) & (coeffs[:, 2] <= nz+1)
        )[0]
    ...
    def _reorder_atoms(self, at_inside_rep: NDArray, a: int, b: int, c:int) -> NDArray:
        ...
        for at in self.at_origin_cell:
            ...
            found = xp.nonzero((xp.linalg.norm(delta, axis=1) < tolerance) & (self.device.atom_type[at] == element_rep))[0]
            if found.size == 0:
                raise ValueError(
    ...
    def _get_atoms_transverse_sorted(self,x:int) -> NDArray:
        ...
        while curr_cell[0] < max_cell[0]:
            while curr_cell[1] < max_cell[1]:
                ...
                at_inside_rep = self._reorder_atoms(at_inside_rep,x, curr_cell[0], curr_cell[1])
                print(f"    " +str(at_inside_rep))
    ...
    def _get_orbitals(self, atoms: NDArray) -> NDArray:
        ...
        for i in range(atoms.shape[0]):
            ...
            index = int(
                atoms[i].get() if hasattr(atoms[i], "get") else atoms[i]
            )
        ...
        while True:
            ...
            for coords in coords_list:
                ...
                if self.device.gamma_only and (self.n_rep_1 > 1 or self.n_rep_2 > 1):
                    H_1 = 0
                    H_2 = 0
                    if (2*radius + 1 ) > self.n_rep_1 or (2*radius + 1) > self.n_rep_2:
                        raise ValueError(
                            f"Error in contact {self.name}: \n"
                ...
                o_1 = a % self.n_rep_1
                o_2 = b % self.n_rep_2
                d1 = self.n_orb_origin_cell*self.n_rep_2 * o_1 + self.n_orb_origin_cell * o_2
                ...
                if (H_1, H_2) in self.device.hamiltonian:
                    ham_read = self.device.hamiltonian[(H_1,H_2)][self.orb_origin_cell,:][:,orb_coup]
                    if ham_read.nnz != 0:
                        self.UC_hamiltonian[(x,coords[0],coords[1])] = ham_read
                        found = True
                if (H_1, H_2) in self.device.overlap:
                    overlap_read = self.device.overlap[(H_1,H_2)][self.orb_origin_cell,:][:,orb_coup]
            ...
            if found == False:
                ...
                if self.n_rep_1 == 1 and self.n_rep_2 == 1 and (radius-1)>0:
                    raise ValueError(
                        f"1x1 UC but more than 1x1 coupling!"
                    )
                break
    ...
    def _residual_coupling(self, orbitals: list) -> bool:
        ...
        tot_orb = xp.arange(self.device.hamiltonian[(0,0)].shape[0])
        tot_orb = tot_orb[~xp.isin(tot_orb, xp.concatenate(orbitals))]
                          
        return self.device.hamiltonian[(0,0)][orbitals[0],:][:,tot_orb].nnz
    ...
    def get_10(self,M):
        ...
        for i in range(self.transverse_rep-1):
            ...
            h10_temp = sparse.vstack([h10_temp, sparse.hstack(hamiltonian_list[-self.transverse_rep:])])
        return h10_temp.T.conj().todense()

    def __init__(self, name, device, vectors: NDArray, origin: NDArray, direction: int, config):
        ...
        print(f"    Number of atoms inside the origin cell: {self.at_origin_cell.shape[0]}")
        print(f"    Number of periodic repetitions in the transverse directions: {self.n_rep_1} x {self.n_rep_2}")
        ...
        for i in range(self.n_rep_1):
            for j in range(self.n_rep_2):
                for k in range(x-1):
                    ...
                    start = self.n_orb_origin_cell * (k*self.n_rep_1*self.n_rep_2 + i*self.n_rep_2 + j)
    ...
    def _upscale_self_energy(self, SE_k: dict, k_1_list: NDArray, k_2_list: NDArray, k1: float, k2: float) -> NDArray:
        ...
        for i in range(self.n_rep_1):
            ...
            for j in range(self.n_rep_2):
                ...
                SE_2.append(xp.zeros_like(SE_k[(k_1_list[0].item(), k_2_list[0].item())]))
        ...
        SE_1 = self._construct_circulant_matrix(SE_1, xp.exp(1j*k1)) / (self.n_rep_1 * self.n_rep_2)
    ...
    def _upscale_injection_modes(self, modes_k: dict, E: NDArray) -> NDArray:
        ...
        for i_E in range(E.shape[0]):
            ...
            modes_E = xp.concatenate(modes_E, axis=1) / xp.sqrt(self.n_rep_1 * self.n_rep_2)
    ...
    def compute_boundary(self, k1: float, k2: float, E) -> None:
        ...
        k_1_list = xp.linspace(0, xp.pi * 2, self.n_rep_1, endpoint=False) + k1 / self.n_rep_1
        k_2_list = xp.linspace(0, xp.pi * 2, self.n_rep_2, endpoint=False) + k2 / self.n_rep_2
        ...
        for ki in k_1_list:
            for kj in k_2_list:
                ...
                for ii in range(self.transverse_rep-1):
                    ...
                    H_tot = sparse.vstack([H_tot,sparse.hstack(H_list, format="csr")], format="csr")
                    S_tot = sparse.vstack([S_tot,sparse.hstack(S_list, format="csr")], format="csr")
                ...
                _ , SE_k[(ki.item(), kj.item())], inj_k[(ki.item(), kj.item())], num_inj_k[(ki.item(), kj.item())], K_k[(ki.item(), kj.item())], T_k[(ki.item(), kj.item())] = self.obc(
                    A_tot[1],
                    A_tot[2],
                    A_tot[0],
                    "left",
