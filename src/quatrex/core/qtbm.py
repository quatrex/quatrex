...
def get_periodic_superblocks_no_flip(
    ...
    if block_sections > 1:
        # Add a zero block
        z_j = _block_view(
            xp.zeros((orb_slab, orb_slab * (block_sections - 1))),
            -1,
            block_sections - 1,
        )
    ...
    if comm.rank == 0:
        with open(filename, "rt") as myfile:
            ...
            for i in range(n):
                ...
                corner1.append(
                    xp.asarray(myfile.readline().split(), dtype=float)
                )  # Read the corner 1
                corner2.append(
                    xp.asarray(myfile.readline().split(), dtype=float)
                )  # Read the corner 2
...
class Contact:
    ...
    vec_atoms_last_block: NDArray = (
        None  # "out-coupling" atoms in the last contact block
    )
    ...
    def sort_cont_at(
        ...
        shifted_coords_first_block = (
            coords[self.vec_atoms_first_block.squeeze(), :] + delta_corner
        )
        coordsType_first_block = coordsType[self.vec_atoms_first_block.squeeze()]

        if (
            vec_atoms.squeeze().shape[0]
            != self.vec_atoms_first_block.squeeze().shape[0]
        ):
            print(
                f"Number of atoms in the shifted contact block ({vec_atoms.squeeze().shape[0]}) does not match the number of atoms in the first block ({self.vec_atoms_first_block.squeeze().shape[0]})"
            )
            raise Exception(
                f"Error: number of atoms in the contact ({vec_atoms.squeeze().shape[0]}) does not match the number of atoms in the first block ({self.vec_atoms_first_block.squeeze().shape[0]})"
            )
        ...
        for i in range(len(vec_atoms)):
            ...
            if numind != 1:
                print(
                    "Number of compatible atoms found in the translated cell: ", numind
                )
                raise Exception(
                    "Error: more than one atom (or no one) found in the translated cell"
                )
    ...
    def compute_vector(
        ...
        orbitals: NDArray,
        coordsType,
        delta_corner: NDArray = xp.array([0, 0, 0]),
    ) -> tuple[NDArray, NDArray]:
        """
        ...
        shifted_corner_1 = (
            self.corner_1 + delta_corner
        )  # Shift the first corner (only needed for the out-coupling elements)
        shifted_corner_2 = (
            self.corner_2 + delta_corner
        )  # Shift the second corner (only needed for the out-coupling elements)
        ...
        if self.vec_atoms_first_block is not None:
            vec_atoms, sorted = self.sort_cont_at(
                vec_atoms, delta_corner, coords, coordsType
            )
        ...
        for i in range(vec_atoms.shape[0]):
            ...
            # I USED A QUICK WORKAROUND FOR NOW
            index = int(
                vec_atoms[i].get() if hasattr(vec_atoms[i], "get") else vec_atoms[i]
            )
            k1 = int(
                orbitals[index].get()
                if hasattr(orbitals[index], "get")
    ...
    def set_vector(
        ...
        self.vec_atoms_first_block, self.vec_orb_first_block, sorted = (
            self.compute_vector(coords, orbitals, coordsType)
        )
        ...
        if sorted == 1:
            (
                print(
                    f"Contact {self.name}, slab {self.N_coup} has {self.vec_atoms_first_block.shape[1]} atoms and {self.vec_orb_first_block.shape[1]} orbitals. Sorted!",
                    flush=True,
                )
                if comm.rank == 0
                else None
            ...
            (
                print(
                    f"Contact {self.name}, slab {self.N_coup} has {self.vec_atoms_first_block.shape[1]} atoms and {self.vec_orb_first_block.shape[1]} orbitals",
                    flush=True,
                )
                if comm.rank == 0
                else None
        ...
        while True:
            ...
            delta_corner = xp.multiply(
                -self.direction * (self.N_coup), (self.corner_2 - self.corner_1)
            )

            # Compute the vector for the new contact block
            vec_atoms_to_append, vec_orb_to_append, sorted = self.compute_vector(
                coords, orbitals, coordsType, delta_corner
            )
            ...
            if ham[self.vec_orb_first_block.T, vec_orb_to_append[None, :]].nnz == 0:
                if self.N_coup_force == -1:  # If yes exit the loop
                    ...
                    if self.N_coup_force < self.N_coup:
                        raise Exception(
                            "Forcing a too small number of contacts element"
                        )
                    if self.N_coup_force == self.N_coup:
                        break
            ...
            self.vec_atoms_cont = xp.concatenate(
                (self.vec_atoms_cont, vec_atoms_to_append)
            )
            self.vec_orb_cont = xp.concatenate((self.vec_orb_cont, vec_orb_to_append))
            if sorted == 1:
                (
                    print(
                        f"Contact {self.name}, slab {self.N_coup} has {vec_atoms_to_append.shape[0]} atoms and {vec_orb_to_append.shape[0]} orbitals. Sorted!",
                        flush=True,
                    )
                    if comm.rank == 0
                    else None
                ...
                (
                    print(
                        f"Contact {self.name}, slab {self.N_coup} has {vec_atoms_to_append.shape[0]} atoms and {vec_orb_to_append.shape[0]} orbitals",
                        flush=True,
                    )
                    if comm.rank == 0
                    else None
        ...
        (
            print(
                f"Contact {self.name} has {self.vec_atoms_cont.shape[1]} atoms and {self.vec_orb_cont.shape[1]} orbitals",
                flush=True,
            )
            if comm.rank == 0
            else None
...
class QTBM:
    ...
    def __init__(
        ...
        self.orbitals_vec = xp.concatenate(
            (xp.array([0]), xp.cumsum(self.orbitals_per_at[self.coordstType])),
            dtype=xp.int32,
        )
        ...
        ) = distributed_load_contact(quatrex_config.input_dir / "cont.dat")
        ...
        self.slab_vec_x_at, self.slab_vec_x_orb = compute_slab_vector_X(
            self.coords, self.n_slabs_x, self.orbitals_vec
        )
        ...
        for n in range(self.n_transmissions):
            ...
            self.observables.electron_transmission_contacts_labels.append(
                self.contacts[cont_1].name[0] + "->" + self.contacts[cont_2].name[0]
            )
            cont_2 += 1
    ...
    def run(self) -> None:
        ...
        for batch_start in range(0, len(self.local_energies), OBC_batch_size):
            ...
            (
                print(
                    f"Time for constructing bare sys. matrix: {t_solve:.2f} s",
                    flush=True,
                )
                if comm.rank == 0
                else None
