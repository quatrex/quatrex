...
class BrillouinZoneMesh:
	...
	def __post_init__(self) -> None:
		object.__setattr__(self, "k_points", _as_float64(self.k_points, name="k_points"))
		object.__setattr__(self, "q_points", _as_float64(self.q_points, name="q_points"))
		object.__setattr__(
			self,
			"frequencies",
...
def infer_periodic_axis(blocks: dict[tuple[int, int, int], NDArray[np.complex128]]) -> int:
...
def build_bloch_hamiltonian(
	...
	*,
	...
	bloch_hamiltonian = np.zeros(
		(k_points_array.size, norb, norb), dtype=np.complex128
	)
	for translation, block in translation_blocks.items():
		phase = np.exp(1j * k_points_array * translation[periodic_axis] * lattice_constant)
...
def map_q_points_to_k_shifts(
	...
	*,
	...
	if k_points_array.ndim != 1 or k_points_array.size < 2:
		raise ValueError("k_points must be a one-dimensional array with at least two points.")
...
def build_uniform_brillouin_zone_mesh(
	*,
	...
	full_q_shifts = np.arange(-num_k_points // 2, (num_k_points + 1) // 2, dtype=np.int64)
...
def compute_rpa_polarization(
	...
	*,
	...
	if form_factors_array.shape != (nq, nk, nbands, nbands):
		raise ValueError(
			"form_factors must have shape (nq, nk, nbands, nbands)."
		)
	if frequencies_array.ndim != 1:
		raise ValueError("frequencies must be a one-dimensional array.")

	energy_differences = (
		energies_kq_array[:, :, np.newaxis, :] - energies_k_array[np.newaxis, :, :, np.newaxis]
	)
	...
	if k_weights is not None:
		...
		integrand = integrand * k_weights_array[np.newaxis, :, np.newaxis, np.newaxis, np.newaxis]
