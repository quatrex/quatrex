...
class RealSpaceOperator:
    ...
    def set_functional(self, functional: FunctionType):
    ...
    def get_dense(self) -> NDArray:
        ...
        if self.matrix is not None:
            ...
            for i in range(len(self.sparsity.row)):
                mat[self.sparsity.row[i], self.sparsity.col[i]] = self.functional(
                    self.positions[self.sparsity.row[i]],
                    self.positions[self.sparsity.col[i]],
                )
    ...
    def get_blockelement_index(self, block: tuple) -> NDArray:
        ...
        return xp.where(
            (self.block_sparsity_map[:, 0] == block[0])
            & (self.block_sparsity_map[:, 1] == block[1])
        )[0]
    ...
    def get_dense_block(self, block: tuple) -> NDArray:
        ...
        if self.matrix is not None:
            mat[
                self.sparsity.row[indices] - self.block_offsets[block[0]],
                self.sparsity.col[indices] - self.block_offsets[block[1]],
            ] = self.matrix[indices]
        elif self.functional is not None:
            for i in range(len(indices)):
                mat[
                    self.sparsity.row[indices[i]] - self.block_offsets[block[0]],
                    self.sparsity.col[indices[i]] - self.block_offsets[block[1]],
                ] = self.functional(
                    self.positions[self.sparsity.row[indices[i]]],
                    self.positions[self.sparsity.col[indices[i]]],
                )
        else:
            raise ValueError("No matrix or functional form set.")
        return mat
    ...
    def set_dense_block(self, block: tuple, matrix_block: NDArray) -> None:
        ...
        if self.matrix is not None:
            indices = self.get_blockelement_index(block)
            self.matrix[indices] = matrix_block[
                self.sparsity.row[indices] - self.block_offsets[block[0]],
                self.sparsity.col[indices] - self.block_offsets[block[1]],
            ]
        else:
    ...
    def get_coo_block(self, block: tuple) -> sparse.coo_matrix:
        ...
        return sparse.coo_matrix(
            (data, (rows, cols)), shape=tuple([self.blocksizes[i] for i in block])
        )
    ...
    def update_sparsity(self, cutoff_distance: float):
        ...
        if (self.matrix is not None) and (self.sparsity is not None):
            ...
            new_matrix_lil = sparse.lil_matrix(
                (self.num_sites, self.num_sites), dtype=self.dtype
            )
            new_matrix_lil[old_coo.row, old_coo.col] = old_coo.data
            self.matrix = (
                new_matrix_lil[new_sparsity.row, new_sparsity.col].toarray().flatten()
            )
    ...
    def get_coo(self) -> sparse.coo_matrix:
        ...
        if self.matrix is not None:
            return sparse.coo_matrix(
                (self.matrix, (self.sparsity.row, self.sparsity.col)),
                shape=(self.num_sites, self.num_sites),
            )
        elif self.functional is not None:
            return sparse.coo_matrix(
                (self.get_matrix_data(), (self.sparsity.row, self.sparsity.col)),
                shape=(self.num_sites, self.num_sites),
            )
        else:
            raise ValueError("No matrix or functional form set.")
...
class RealSpacePairOperator:
    ...
    def set_functional(self, functional: FunctionType):
        ...
        ----------
        functional : function
            The functional form of the operator, takes as input positions of four sites.
    ...
    def get_dense(self) -> NDArray:
        ...
        if self.matrix is not None:
            mat[self.pair_sparsity.row, self.pair_sparsity.col] = self.matrix
            return mat
        elif self.functional is not None:
            for i in range(len(self.pair_sparsity.row)):
                mat[self.pair_sparsity.row[i], self.pair_sparsity.col[i]] = (
                    self.functional(
                        self.positions[self.sparsity.row[self.pair_sparsity.row[i]]],
    ...
    def get_coo(self) -> sparse.coo_matrix:
        ...
        if self.matrix is not None:
            return sparse.coo_matrix(
                (self.matrix, (self.pair_sparsity.row, self.pair_sparsity.col)),
                shape=(self.nni, self.nni),
            )
        elif self.functional is not None:
            return sparse.coo_matrix(
                (
                    self.get_matrix_data(),
                    (self.pair_sparsity.row, self.pair_sparsity.col),
                ),
                shape=(self.nni, self.nni),
            )
        else:
...
def _compute_block_map(sparsity: sparse.coo_matrix, blocksizes: NDArray) -> NDArray:
    """Computes the block map of the sparsity pattern for a given block sizes.
