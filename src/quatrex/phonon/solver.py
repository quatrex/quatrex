...
class PhononSolver(SubsystemSolver):
    ...
    def __init__(
        ...
        # Load the device dynamical matrix.
        synchronize_device()
        comm.barrier()
        t_ham_load_start = time.perf_counter()
        if quatrex_config.device.construct_from_unit_cell:
            dynmat_unit_cells = distributed_load(
                quatrex_config.input_dir / "dynmat_unit_cells.npy"
            ...
            dynmat_sparray, block_sizes = create_hamiltonian(
                cutoff_hr(
                    dynmat_unit_cells,
            ...
            dynmat_sparray = dynmat_sparray.astype(xp.complex128)
            dynmat_sparray.sum_duplicates()
            ...
            dynmat_sparray = distributed_load(
                quatrex_config.input_dir / "dynmat.npz"
            ).astype(xp.complex128)
        ...
        # self-energy and dynmat sparsity.
        if sparsity_pattern is None:
            sparsity_pattern = dynmat_sparray.copy()
        else:
            sparsity_pattern += dynmat_sparray
        ...
        if comm.rank == 0:
            print(
                f"    Load dynmat: {t_ham_load_end-t_ham_load_start}",
                flush=True,
            )
            print(
                f"    Load dynmat all: {t_ham_load_end_all-t_ham_load_start}",
                flush=True,
            )

        self.dynmat = compute_config.dsdbsparse_type.from_sparray(
            dynmat_sparray.astype(xp.complex128),
        ...
        if comm.rank == 0:
            print(
                f"    Create dynmat: {t_ham_create_end-t_ham_load_end_all}",
                flush=True,
            )
            print(
                f"    Create dynmat all: {t_ham_create_end_all-t_ham_load_end_all}",
                flush=True,
            )
        ...
        # Check that the provided block sizes match the dynmat.
        if self.block_sizes.sum() != self.dynmat.shape[-2]:
            raise ValueError(
                "Block sizes do not match dynmat. "
                f"{self.block_sizes.sum()} != {self.dynmat.shape[-2]}"
            )
        ...
        # Check that the overlap matrix and dynmat matrix match.
        if self.overlap_sparray.shape != self.dynmat.shape[-2:]:
            raise ValueError(
                "Overlap matrix and dynmat matrix have different shapes."
            )

        # Make sure that the dynmat and overlap matrices are
        # Hermitian.
        if not self.dynmat.symmetry:
            self.dynmat.symmetrize()
        self.overlap_sparray = (
            0.5 * (self.overlap_sparray + self.overlap_sparray.conj().T)
        ).tocoo()

        self.eta = quatrex_config.phonon.eta

        # Contacts.
        self.flatband = quatrex_config.phonon.flatband
        if self.flatband and comm.rank == 0:
            print("Flatband conditions detected", flush=True)

        self.eta_obc = quatrex_config.phonon.eta_obc

        if quatrex_config.phonon.solver.compute_current and comm.block.size > 1:
            raise NotImplementedError(
                "Current computation not implemented in distributed mode."
            )

        self.compute_meir_wingreen_current = (
            quatrex_config.phonon.solver.compute_current
        )

        self.dos_peak_limit = quatrex_config.phonon.dos_peak_limit

        self.temperature = quatrex_config.phonon.temperature

        self.left_occupancies = bose_einstein(
            self.local_energies, self.temperature
        )
        ...
        self.block_sections = quatrex_config.phonon.obc.block_sections
    ...
    def load_dynmat(
        ...
        return dynmat_sparray, block_sizes
    ...
    def _assemble_system_matrix(self, sse_retarded: DSDBSparse) -> None:
        ...
        scale_stack(
            ...
            self.local_energies**2 + 1j * self.eta,
        )

        _btd_subtract(self.system_matrix, sse_retarded)
        _btd_subtract(self.system_matrix, self.dynmat)
