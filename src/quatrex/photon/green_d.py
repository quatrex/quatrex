...
class PhotonSolver(SubsystemSolver):
    ...
    def __init__(
        ...
        self.local_energies = get_local_slice(
            photon_energies, comm.stack
        )  # Each process gets a subset — its “local” portion of the total energy grid — to work on in parallel.
        ...
        self.D0_delta = D0_delta_product(
            distance_unit_cells, self.local_energies
        ).astype(
            xp.complex128,
            copy=False,
        )
        ...
        # TODO: Naming Photon_matrixw not sure the best for coherence
        ...
        self.num_connected_blocks = quatrex_config.photon.num_connected_blocks
    ...
    def solve(
        ...
        if comm.rank == 0:
            ...
            self._assemble_system_matrix(
                p_retarded, self.D0_matrix
            )  # UPDATES the values matrix system_matrix
...
if __name__ == "__main__":
    ...
    d_solver = PhotonSolver(
        ...
        sparsity_pattern=sp.coo_matrix(
            (N, N)
        ),  # Placeholder; not used in this test harness.
    )
    ...
    for idx, (i, j) in enumerate(zip(rows, cols)):
        ...
        p_greater[:, i, j, :, :] = random_matrix_d[
            :, idx, :, :
        ].conj()  # Just for testing
        p_retarded[:, i, j, :, :] = 0.5 * (
            p_lesser[:, i, j, :, :] - p_greater[:, i, j, :, :]
        )  # Just for testing
