...
from qttools import xp, NDArray
...
def load_distances(config: IOConfig) -> NDArray:
    ...
    # else:
        ...
        print("example_input_dir was required because we are not constructing from thes unit cell yet")
        orbital_positions = xp.load(config.example_input_dir / "grid.npy")
        distance_unit_cells = (orbital_positions[:, None, :] - orbital_positions[None, :, :]).astype(xp.complex128, copy=False)
...
def load_hamiltonian_sparse(config: IOConfig) -> tuple[DSDBSparse, NDArray]:
    ...
    if config.device.construct_from_unit_cell:
        ...
        block_sizes = xp.asarray(
            [block_sizes[0]] * config.device.number_of_supercells
        )

    else: 
        print("example_input_dir was required because we are not constructing from the unit cell yet")
        hamiltonian_sparray = sp.load_npz(config.example_input_dir / "hamiltonian.npz").tocsr().astype(xp.complex128, copy=False)
