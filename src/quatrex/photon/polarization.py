...
def _as_dense(a):
    """Very light wrapper: make sure we hand dense arrays to FFT/einsum."""
    if isinstance(a, DSDBSparse):
        # adjust if your DSDBSparse exposes another attribute for dense data
        return a.toarray()
    return a
...
class PiPhoton(ScatteringSelfEnergy):
    ...
    def __init__(
        ...
        self.Ne = len(self.energies) #discretization goes from 0 to Ne-1 (have to add padding)
    ...
    def compute(
        ...
        p_greater[...] = xp.conj(p_lesser[::-1].transpose(0,2,1,4,3)) #reorders the axes : to keeps axis 0 (energy) first,then swaps 1↔2 (i ↔ j) and 3↔4 (u ↔ v).
...
if __name__ == "__main__":
    ...
    g_greater = xp.zeros((num_electron_energies, num_orbitals, num_orbitals), dtype=complex)
    ...
    # outputs
    P_less = xp.empty((num_photon_energies, num_orbitals, num_orbitals, 3, 3), dtype=complex)
